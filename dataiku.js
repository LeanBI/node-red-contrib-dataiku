    ...
    function AccelerometerNode(config) {
        ...
        function getData() {
        	if (spatial.acceleration.length>0) {
        		...
        		node.log("no data received (device not ready ?) \n" + JSON.stringify(spatial,null,2))
        ...
        //spatial.phidget.params.host="192.168.178.30"
        conn=spatial.connect()
        node.log("connecting " + JSON.stringify(spatial,null,2))
